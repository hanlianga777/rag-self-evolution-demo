...
`/api/preview` 会始终返回结构化 evidence；若没有真实命中，它会拒答且 evidence 为空，不会调用 Provider。每个命中以 API 原点的 `http://localhost:8010/documents/...pdf#page=N` 打开原生 PDF，避免 Vite 的 SPA 回退误替代 PDF。`start_demo.command` 是 Finder 可双击的入口：它安装登录启动项，并以隐藏的 Terminal 上下文启动后脱离 Terminal 的服务；日志位于忽略的 `.demo-logs/`，且不写入 `.env`。`/api/evaluations/live` 仅在响应 `mode=live` 时代表有效模型回答；Demo Active 保存于 SQLite 独立状态，`/api/experiments/*` 始终是 `mode=mock`、`source=seeded_replay` 的演示回放，不代表真实实验。
