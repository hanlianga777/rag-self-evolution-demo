...
export function SettingsPage({ data }: { data: any }) {
  ...
  const verify = async () => { setChecking(true); setError(""); setProbe(null); try { setProbe(await operation.run("验证 DeepSeek Provider", () => postJson("/api/ai-readiness/probe"))); } catch (reason) { setError(errorMessage(reason)); } finally { setChecking(false); } };
