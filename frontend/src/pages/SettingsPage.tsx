...
export function SettingsPage({ data }: { data: any }) {
  ...
  const verify = async () => { setChecking(true); setError(""); setProbe(null); try { setProbe(await operation.run("验证 DeepSeek Provider", async () => { const result: any = await postJson("/api/ai-readiness/probe"); if (result.probe === "failed") throw new Error(result.last_probe?.reason || "Provider 连接失败"); return result; })); } catch (reason) { setError(errorMessage(reason)); } finally { setChecking(false); } };
  return <div className="page settings-page"><div className="page-title"><div><h1>设置</h1><p>此工作区的 Provider 与评测设置。</p></div></div><div className="settings-grid"><Section title="LLM Provider"><dl><dt>Provider</dt><dd>DeepSeek</dd><dt>模型</dt><dd>{r.model || "未配置"}</dd><dt>处理方式</dt><dd><Badge tone="neutral">{displayText(r.mode)}</Badge></dd><dt>API 状态</dt><dd><Badge tone="neutral">{status}</Badge></dd></dl><p className="muted">验证连接会向 DeepSeek 发送固定测试消息，不发送文档内容。</p><button className="secondary" onClick={verify} disabled={checking}>{checking ? "正在验证…" : "验证 Provider 连接"}</button>{error && <p className="error-notice" role="alert">验证失败：{error}。请重试。</p>}{audit && <p className="muted">最后验证：{displayText(audit.status)}{audit.latency_ms != null ? ` · ${audit.latency_ms} ms` : ""}{audit.reason ? ` · ${audit.reason}` : ""}</p>}{r.reason && <p className="muted">{r.reason}</p>}</Section><Section title="检索"><dl><dt>模式</dt><dd>CandidateK → Hybrid → Rerank → MinScore → TopK</dd><dt>语料</dt><dd>机器人官方 PDF 文档</dd><dt>数据边界</dt><dd>不上传原文件；相关知识片段可能发送给 DeepSeek。</dd></dl></Section><Section title="评测"><dl><dt>Judge</dt><dd>DeepSeek JSON Judge</dd><dt>数据集</dt><dd>Golden Dataset · 仅已批准题目可运行</dd><dt>数据边界</dt><dd>Judge 还会接收问题、预期回答和生成回答。</dd></dl></Section><Section title="评分与 SLA"><dl><dt>Overall Score</dt><dd>Positive / Ablation / Safety 9 项质量指标等权平均，仅展示和横向比较。</dd><dt>Release Gate</dt><dd>11 / 11 Hard Gate + Regression；Overall Score 不能替代 Gate。</dd></dl></Section></div></div>;
}
