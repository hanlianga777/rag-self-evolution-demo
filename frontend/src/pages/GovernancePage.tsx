...
export function GovernancePage({ data }: { data: any }) {
  ...
  const batchReason = current.length !== 20 ? "本轮尚未完整入库 20 道题" : gateBlocked.length || manualBlocked.length ? `仍有 ${new Set([...gateBlocked, ...manualBlocked].map(row => row.id)).size} 道阻塞题（Probe 未通过 ${probeFailed}，QC 未通过 ${qcFailed}${probePending ? `，待 Probe ${probePending}` : ""}${qcPending ? `，待 QC ${qcPending}` : ""}${manualBlocked.length ? `，人工需修订 / 已拒绝 ${manualBlocked.length}` : ""}）` : current.every(row => row.stage === "golden") ? "本轮全部题目已批准" : !confirmed ? "请先确认已人工审核全部有效 Candidate" : "";
...
function CandidateDetail({ row, peers, revision, rerunSlot, busy, onRun, onReview, onRefresh, operation }: { row?: Candidate; peers: Candidate[]; revision?: Candidate; rerunSlot?: Candidate; busy: boolean; onRun: (id: string, name: "probe" | "qc") => void; onReview: (id: string, decision: string, reason?: string, tags?: string[]) => void; onRefresh: () => Promise<void>; operation: ReturnType<typeof useOperation> }) {
  ...
  useEffect(() => { setRevisionRun(revision); setPreviewTarget(row?.id || ""); setPreviewEditing(false); setPreviewBoth(false); setPaired(false); }, [revision?.id, row?.id]);
  useEffect(() => {
  ...
  const beginRevision = async () => {
    ...
    setRevisionBusy(true); setRevisionError("");
    try {
      const payload = { mode, reason, paired: !!linked && paired, changes: Object.fromEntries(selectedRows.map(item => [item.id, { source_chunk_ids: item.test_category === "negative" ? [] : changes[item.id]?.source_chunk_ids ?? item.evidence?.flatMap((source: Candidate) => source.source_chunk_ids || []) ?? [] }])), tags: tags.split(/[，,]/).map(value => value.trim()).filter(Boolean) };
  ...
  const expected = new Set((row.evidence || []).flatMap((item: Candidate) => item.source_chunk_ids || []));
  const rank = topK.findIndex((item: Candidate) => expected.has(item.chunk_id));
  return <div className="drawer-body candidate-detail">
    <h3>{row.slot || row.id} · {row.question}</h3><dl><dt>Question ID</dt><dd>{row.id}</dd><dt>类型</dt><dd>{displayText(row.test_category)}</dd><dt>属性 / 负向子类</dt><dd>{displayText(row.raw?.ablation_attribute || row.raw?.negative_subtype || row.raw?.expected_behavior || "—")}</dd></dl>
    ...
    {row.stage !== "golden" && ["needs_revision", "rejected"].includes(row.review_status) && !activeRevision && <div className="revision-form"><h4>发起 Candidate 修订</h4><label>修订原因<input value={reason} onChange={event => setReason(event.target.value)} placeholder="请写明本次修订原因" /></label><div className="header-actions"><span>Revision Mode</span><label><input type="radio" checked={mode === "manual_edit"} onChange={() => setMode("manual_edit")} /> 人工编辑</label><label><input type="radio" checked={mode === "ai_regenerate"} onChange={() => setMode("ai_regenerate")} /> AI 单 Slot 重生成</label></div>{linked && <div className="revision-pair"><strong>关联 {row.slot === "Q09" ? "Positive" : "Ablation"}：{linked.slot}</strong><label><input type="checkbox" checked={paired} disabled={linked.stage === "golden" || !["needs_revision", "rejected"].includes(linked.review_status)} onChange={event => setPaired(event.target.checked)} /> 同时修订关联 {row.slot === "Q09" ? "Positive" : "Ablation"} {linked.slot}</label>{linked.stage === "golden" && <p className="muted">关联题已批准，不可共同修订。</p>}</div>}{selectedRows.map(item => item.test_category !== "negative" && <fieldset key={item.id}><legend>{item.slot} · 真实 Evidence Chunk</legend><div className="revision-chunks">{chunks.filter(chunk => item.evidence_details?.[0]?.chunks?.[0]?.document_id === chunk.document_id).map(chunk => { const selected = changes[item.id]?.source_chunk_ids ?? item.evidence?.flatMap((source: Candidate) => source.source_chunk_ids || []) ?? []; return <label key={chunk.chunk_id}><input type="checkbox" checked={selected.includes(chunk.chunk_id)} onChange={event => edit(item.id, "source_chunk_ids", event.target.checked ? [...selected, chunk.chunk_id] : selected.filter((id: string) => id !== chunk.chunk_id))} /> {chunk.chunk_id} · {chunk.section_path} · P.{chunk.page_start}<small>{chunk.chunk_text || chunk.text}</small></label>; })}</div></fieldset>)}<button className="primary" disabled={revisionBusy || !reason.trim()} onClick={() => void beginRevision()}>生成修订草案</button></div>}
    ...
    {revisionRun && <div className="revision-status" role="status">
      ...
      {revisionRun.drafts && ["preview_ready", "generating", "validating", "interrupted"].includes(revisionRun.status) && <>
        ...
        {revisionRun.status === "preview_ready" && <>
          <label>选择要调整的题目 <select aria-label="选择要调整的题目" value={selectedPreviewIds[0]} onChange={event => { setPreviewTarget(event.target.value); setPreviewBoth(false); }}>{revisionRun.question_ids.map((id: string) => <option key={id} value={id}>{revisionRun.before[id].raw?.coverage_slot || id}</option>)}</select></label>
          ...
          <div className="header-actions"><button className="secondary" disabled={revisionBusy} onClick={openPreviewEdit}>编辑当前草案</button><button className="secondary" disabled={revisionBusy || previewBoth} onClick={() => void regeneratePreview()}>重新生成当前题</button><button className="secondary" disabled={revisionBusy} onClick={() => void discardPreview()}>放弃草案</button></div>
