...
export function GovernancePage({ data }: { data: any }) {
  ...
  const batchReason = current.length !== expectedCount ? `本轮尚未完整入库 ${expectedCount} 道题` : gateBlocked.length || manualBlocked.length ? `仍有 ${new Set([...gateBlocked, ...manualBlocked].map(row => row.id)).size} 道题需逐题处理或接受 QC P0 风险` : current.every(row => row.stage === "golden") ? "本轮已确认 Golden 测试集" : !confirmed ? "请先确认已人工审核全部有效 Candidate" : "";
  ...
  return <div className={`page governance-page ${!currentRun && tab === "run" ? "is-empty" : ""}`}>
    <div className="page-title"><div><h1>测试集治理</h1><p>Coverage → Hard Validation → Probe → QC → Gate 1 人工确认 Golden</p></div><button className="secondary" disabled={busy || running || qualityRunning} onClick={() => void createMini()}>{currentRun?.status === "failed" ? "重新运行 V1 Mini 8 / 4 / 8" : "生成 V1 Mini 8 / 4 / 8"}</button></div>
    ...
    {tab === "run" && <Section title="当前 V1 Generation Run" action={<div className="header-actions">{currentRun && <Badge tone="accent">{currentRun.id}</Badge>}{current.length === expectedCount && currentRun?.status === "completed" && <button className="secondary" disabled={busy || qualityRunning} onClick={() => void rerunQuality()}>重新运行 Probe / QC</button>}{current.length === expectedCount && <details className="export-menu"><summary className="secondary">导出测试集 ▾</summary><div>{(["markdown", "csv", "json"] as const).map(format => <a key={format} href={apiUrl(`/api/governance/generation-runs/${currentRun.id}/export?format=${format}`)} download>导出 {format === "markdown" ? "Markdown" : format.toUpperCase()}</a>)}</div></details>}</div>}>
      {currentRun ? <>
        {current.length === expectedCount && current.every(row => row.stage === "golden") && <div className="snapshot-next"><span>✓ Gate 1 已确认 Golden 测试集；冻结版本可在历史版本查看。</span></div>}
        ...
        {current.length === expectedCount && !current.every(row => row.stage === "golden") && <div className="review-batch"><label><input type="checkbox" checked={confirmed} onChange={event => setConfirmed(event.target.checked)} /> 我已人工审核本次 V1 Mini 的全部有效 Candidate</label><button className="primary" disabled={busy || !!batchReason} onClick={() => void action(() => postJson("/api/governance/review-batch", { question_ids: currentIds, confirmed_manual_review: true }), "确认 Golden 测试集")}>确认 Golden 测试集</button>{batchReason && <span className="muted">{batchReason}</span>}</div>}
