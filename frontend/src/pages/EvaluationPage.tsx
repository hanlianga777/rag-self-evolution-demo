...
export function EvaluationPage({ data, navigate, selectedCaseId, onSelectedCaseOpened }: { data: any; navigate: (page: any) => void; selectedCaseId?: string; onSelectedCaseOpened?: () => void }) {
  ...
  return <div className="page"><div className="page-title"><div><div className="eyebrow">基线评测 · 运行 #{evaluation.id}</div><h1>评测报告</h1><p>{evaluation.config} · {evaluation.dataset} · {evaluation.questions} 道问题 · <Status value={evaluation.status} /></p></div><div className="header-actions"><button className="secondary" onClick={runLiveEvaluation} disabled={runningLive}>{runningLive ? "正在运行真实评测…" : "运行真实评测"}</button><button className="primary" onClick={() => navigate("evolution")}>优化本次运行 <ArrowRight size={16} /></button></div></div><p className="notice">手动运行真实评测会把问题与相关知识片段发送给 DeepSeek；Judge 还会接收预期回答与生成回答。不上传原文件。</p>{error && <p className="error-notice" role="alert">评测失败：{error}。请重试。</p>}{liveRun && <div className="notice">{liveRun.mode === "live" ? "真实评测结果（Live）" : "模拟结果（Mock，未执行真实评测）"} · 完成 {liveRun.completed} · 失败 {liveRun.failed} · 平均分 {liveRun.average_score ?? "—"} · {liveRun.latency_ms} ms{liveRun.reason ? ` · 原因：${liveRun.reason}` : ""}</div>}
    ...
    <Section title="8 个问题案例" action={<div className="filters"><Filter size={14} />{filters.map(([value, label]) => <button key={value} className={filter === value ? "selected" : ""} onClick={() => setFilter(value)}>{label}</button>)}</div>}><div className="table-scroll" role="region" aria-label="问题案例表" tabIndex={0}><table><thead><tr><th>案例 ID</th><th>问题</th><th>失败类型</th><th>评分</th><th>严重程度</th><th>状态</th></tr></thead><tbody>{visible.map((item: any) => <tr key={item.id}><td><button className="text-button detail-button" onClick={() => setSelected(item)}>{item.id}</button></td><td>{item.question}</td><td>{displayText(item.failure_type)}</td><td>{item.score}</td><td><Badge tone={item.severity === "High" ? "bad" : "warning"}>{displayText(item.severity)}</Badge></td><td><Status value={item.status} /></td></tr>)}</tbody></table></div></Section>
