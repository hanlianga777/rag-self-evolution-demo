...
export function VersionsPage({ data }: { data: any }) {
  ...
  return <div className={`page versions-page ${!candidates.length ? "is-empty" : ""}`}>
    <div className="page-title"><div><h1>版本与发布</h1><p>完整 Round、Sandbox、11 项 Hard Gate、Regression 与 Recommendation 通过后，由人确认发布。</p></div></div>
    {error && <p className="error-notice" role="alert">{error}</p>}
    <Section title="待发布 Candidate"><div className="release-list">
      {candidates.map(candidate => { const state = candidate.release_state || {}; const checks = [["Round 完整", state.round_complete], ["Sandbox", state.sandbox], ["Qualified（含 Gate 与 Regression）", state.qualified], ["Recommendation", state.recommended], ["Human Release", state.human_release]]; return <article className="release-row" key={candidate.id}>
