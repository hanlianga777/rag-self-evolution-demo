...
export function OverviewPage({ data, navigate }: { data: any; navigate: (page: any) => void }) {
  ...
  return <div className="page"><div className="eyebrow">园区运营知识助手</div><div className="page-title"><div><h1>RAG 质量概览</h1><p>由评测驱动，从基线优化到经验证的候选方案。</p></div><div className="production"><span>生产配置</span><strong>v1.0 基线</strong></div></div>
    ...
    <Section title="进化流程" action={<Badge tone="accent"><Sparkles size={12} /> 评测驱动</Badge>}><div className="pipeline">{item.pipeline.map((node: any, index: number) => <button className="pipeline-node" key={node.label} onClick={() => navigate(index < 2 ? "evaluation" : index < 6 ? "evolution" : "versions")}><span>{displayText(node.label)}</span><strong>{displayText(node.value)}</strong>{index < item.pipeline.length - 1 && <ArrowRight className="pipeline-arrow" size={15} />}</button>)}</div></Section>
    <div className="two-column"><Section title="问题案例分布"><div className="chart"><ResponsiveContainer width="100%" height={210}><BarChart layout="vertical" data={item.distribution} margin={{ left: 28 }}><XAxis type="number" hide /><YAxis type="category" dataKey="name" width={120} tickFormatter={displayText} tick={{ fontSize: 12, fill: "#667085" }} /><Bar dataKey="value" radius={[0, 5, 5, 0]} fill="#4757c7" /></BarChart></ResponsiveContainer></div></Section><Section title="最新优化结果"><div className="candidate-list">{item.latest_optimization.map((candidate: any) => <div className="candidate-row" key={candidate.name}><span>{displayText(candidate.name)}</span><strong>{candidate.score}</strong>{candidate.name === "Candidate B" && <Badge tone="good"><CheckCircle2 size={12} /> 推荐</Badge>}</div>)}</div></Section></div>
