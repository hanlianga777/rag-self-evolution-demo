...
export function EvolutionPage({ data }: { data: any }) {
  ...
  return <div className="page evolution-page">
    ...
    {stage === "agent" && <Section title="2. 优化 Agent" action={<button className="secondary" onClick={() => setSearchOpen(true)}>查看 Search Space</button>}><div className="pipeline"><div className="pipeline-node"><span>Optimization Run</span><strong>{experiment?.status || "未运行"}</strong></div><div className="pipeline-node"><span>Sandbox 预算</span><strong>{budget.used} / {budget.max}（为 D 预留 {budget.reserved_for_d ?? budget.reserved_for_d1 ?? 1} 次）</strong></div><div className="pipeline-node"><span>当前 Round</span><strong>{latestRound ? `Round ${latestRound.round} · ${latestRound.evaluated}/${latestRound.total ?? 3}` : "未运行"}</strong></div></div></Section>}
