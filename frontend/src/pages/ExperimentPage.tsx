...
export function ExperimentPage({ onOpenCitation }: { onOpenCitation: (citation: Citation) => void }) {
  ...
  const candidateRows = candidate.result ? [["版本", candidate.result.version], ["回答模式", candidate.result.mode === "live" ? "服务回答" : "本地响应"], ["生成模型", candidate.result.model ?? "未返回有效模型回答"], ["处理说明", candidate.result.fallback_reason || "无"], ["处理延迟", `${candidate.result.latency_ms} ms`], ["Top K", "4"], ["检索", "BGE + FAISS"]] : [["版本", "v1.2"], ["回答模式", "等待提问"], ["生成模型", "未运行"], ["处理说明", candidate.error || "无"], ["处理延迟", isThinking(candidate) ? "正在思考" : "未运行"], ["Top K", "未运行"], ["检索", "未运行"]];
