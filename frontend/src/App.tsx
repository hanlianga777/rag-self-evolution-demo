...
const navigation: { id: Page; label: string; icon: typeof BarChart3 }[] = [
  { id: "assistant", label: "园区助手", icon: MessageCircle }, { id: "overview", label: "概览", icon: BarChart3 }, { id: "knowledge", label: "知识与数据集", icon: BookOpen }, { id: "evaluation", label: "评测", icon: Waypoints }, { id: "evolution", label: "进化实验室", icon: FlaskConical }, { id: "versions", label: "版本管理", icon: Sparkles },
];

export default function App() {
  const [data, setData] = useState<AppData | null>(null); const [error, setError] = useState(""); const [page, setPage] = useState<Page>("assistant"); const [preview, setPreview] = useState(false); const [openedCaseId, setOpenedCaseId] = useState<string>();
  ...
  const content = page === "assistant" ? <AssistantPage badCases={data.badCases} onOpenBadCase={caseId => { setOpenedCaseId(caseId); setPage("evaluation"); }} /> : page === "overview" ? <OverviewPage data={data} navigate={setPage} /> : page === "knowledge" ? <KnowledgePage data={data} /> : page === "evaluation" ? <EvaluationPage data={data} navigate={setPage} selectedCaseId={openedCaseId} onSelectedCaseOpened={() => setOpenedCaseId(undefined)} /> : page === "evolution" ? <EvolutionPage data={data} /> : page === "versions" ? <VersionsPage data={data} onActivated={refreshVersions} /> : <SettingsPage data={data} />;
  const workspace = data.workspace as any;
  return <div className="app-shell"><aside><div className="brand"><Bot size={21} /><span>RAG Evolution</span></div><div className="nav-label">工作区</div><nav aria-label="主导航">{navigation.map(item => { const Icon = item.icon; return <button key={item.id} className={page === item.id ? "active" : ""} onClick={() => setPage(item.id)}><Icon size={17} />{item.label}</button>; })}</nav><div className="sidebar-bottom"><button className={page === "settings" ? "active" : ""} onClick={() => setPage("settings")}><Settings size={17} />设置</button><div className="persona">AI 解决方案工程<br /><span>运营工作区</span></div></div></aside><main className="main"><header><button className="secondary mobile-menu" aria-expanded={menuOpen} aria-controls="mobile-navigation" onClick={() => setMenuOpen(!menuOpen)}><Menu size={16} />菜单</button><div className="workspace-select"><span>{workspace.name}</span><small>{displayText(workspace.environment)}</small></div><div className="header-actions"><Badge tone="neutral">{data.readiness.mode === "live" ? "服务已连接" : "本地响应"}</Badge><button className="secondary" onClick={() => setPreview(true)}><Eye size={16} />回答对比</button></div></header>{menuOpen && <nav id="mobile-navigation" className="mobile-navigation" aria-label="移动导航">{[...navigation, { id: "settings" as Page, label: "设置", icon: Settings }].map(item => <button key={item.id} aria-current={page === item.id ? "page" : undefined} onClick={() => { setPage(item.id); setMenuOpen(false); }}>{item.label}</button>)}</nav>}{content}</main><Preview open={preview} onOpenChange={setPreview} /></div>;
}
