...
it("accepts a reviewable QC P0 with a recorded reason even when scores are low", async () => {
  ...
  const row = { id: "Q1", slot: "Q01", question: "问题", test_category: "positive", stage: "candidate", review_status: "human_review_pending", probe_status: "needs_revision", qc_status: "qc_failed", probe: { score: 72 }, qc: { score: 70 }, evidence: [], approval_eligibility: { can_approve: true, blocking_reasons: [], requires_qc_p0_acceptance: true, qc_reason: "证据支持待人工判断", qc_created_at: "2026-09-27T00:00:00Z" } };
...
it("sends one atomic Gate 1 review request and shows the frozen version after refresh", async () => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (url: string, init?: RequestInit) => {
    ...
    const body = url.includes("export") ? { questions: rows } : url.endsWith("/api/dataset") ? rows : url.endsWith("/api/governance/generation-runs") ? [run] : url.endsWith("/api/governance/snapshots") ? [{ id: "GOLDEN-v1" }] : [];
