    ...
    const timer = window.setInterval(() => {
      for (const item of active) {
        void getJson<any>(item.kind === "revision" ? `/api/governance/revisions/${item.id}` : `/api/evaluations/${item.id}`).then(result => {
          const next: Partial<Operation> = item.kind === "revision" ? { status: result.apply_blocked || ["failed", "failed_quality", "interrupted"].includes(result.status) ? "failed" : ["preview_ready", "completed", "cancelled"].includes(result.status) ? "completed" : "running", stage: ({ queued: "等待草案", generating: "AI 单题生成", validating: "Hard Validation", probing: "Probe", qc: "QC", preview_ready: "草案待确认应用", completed: "待人工复审", cancelled: "草案已放弃", interrupted: "进程已中断，需手动继续" } as Record<string, string>)[result.status] || result.stage, current: result.progress?.current, total: result.progress?.total, error: result.error || (result.status === "interrupted" ? "进程重启后 Worker 不会自动恢复" : undefined) } : evaluation(result);
          update(item.id, next);
