...
- **Golden：** 当前只支持 Mini 8 Positive / 4 Ablation / 8 Negative。Candidate 经 Hard Validation、Probe ≥90、QC ≥85 和人工审核后才可进入不可变 Golden Snapshot；修订单题独立进行，关联由元数据表达。历史 40 题和旧 V1 Run 不计入当前 KPI。
...
- **当前数据边界：** 最近完整 Run 有 20 道题（16 已批准、4 需修订），尚无正式 Golden Snapshot；`baseline-v1` 仅为初始配置，不是正式发布记录。实际数量以 SQLite/API 实时记录为准。项目不以 Seed、Mock 或 Fixture 冒充 Golden、评测、推荐或发布成果。
