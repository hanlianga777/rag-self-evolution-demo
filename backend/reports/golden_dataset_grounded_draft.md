# Grounded Golden Dataset Candidate Draft

本报告为基于正式 PDF Evidence Inventory 的 32 道候选草案，不是正式 Evaluation Dataset；未修改 Evaluation、Retrieval、Chunking、Agent、`seed.py` 或 Demo。
...
- grounded_single：24
- grounded_multi：8
- 产品覆盖（按每题首个 `product_scope`，即主 Evidence 计）：KIRA B 50 12；B2 四足机器人 10；B2 遥控器 6；B2 电池与充电器 4。
- Knowledge Type 覆盖：charging 5；maintenance 6；operation 11；safety 4；setup 11；specification 1；troubleshooting 3；usage_limit 3。
- High Criticality：17
- 疑似重复题：0（按规范化 Reference Answer 与 Key Points 对 496 个题对交叉检查）。
- Evidence Check：32 / 32。
- R1/R3 版本冲突：未使用“充满指示”事实；GGC-029 只采用两份资料均不冲突的充电前提与连接顺序。
