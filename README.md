...
配置状态区分“已配置（未验证）”“已验证可用”和“不可用”。打开问答试验不会自动请求，需点击“发送”；页面会并行调用 `POST /api/preview/baseline` 与 `POST /api/preview/candidate`，各自收到响应后立即开始输出，并只显示该接口实际测得的 `latency_ms`。原有 `POST /api/preview` 保持兼容，供 AI 问答等既有调用使用。基线始终是模拟样例；任一列失败只清除该列结果并允许重试。
