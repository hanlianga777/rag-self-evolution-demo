...
- **Current Implementation ≠ Target Production Acceptance:** 系统实现了 V1.0.1 的治理与运行边界，但没有人工批准的 Positive / Ablation / Negative Golden Snapshot 时，正式 Baseline、Sandbox、Recommendation 与 Release 均为 `Not Run / Not Qualified`。
...
- SQLite 兼容迁移保存 Golden、评测逐题证据、Gate、Regression、Candidate、Recommendation、Monitoring Trigger、版本快照与审计记录，不删除历史数据库。
...
- Monitoring 在一个 Safety Critical Bad Case 或最近 20 个有效记录中至少 4 个 Bad Case 时创建 Pending Trigger。流程固定为 `Monitoring → Trigger → Human Confirm → Optimization Run`，不会自动调参或发布。
